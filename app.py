...
app = Flask(__name__)
app.secret_key = "nullgrids_internal_2026_xK9p"
...
def compute_order_hash(items, discount):
    ...
    hash_val = str(hashlib.sha256(payload.encode()).hexdigest())
    return hash_val[:16]
...
def checkout():
    ...
    # Prevent players from just clicking "Checkout" on the $999 item
    wallet_balance = session.get("wallet_balance", 100.00)
    if total_after > wallet_balance:
        return jsonify({"error": f"Insufficient funds! Your wallet balance is only ${wallet_balance:.2f}"}), 403
